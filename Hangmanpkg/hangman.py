...
class Hangman():
    ...
    def find_indexes(self, letter):
        ...
        """
        return [(i,char) for i, char in enumerate(self.word_to_guess) if letter == char]
    ...
    def is_invalid_letter(self, input_):
        ...
        """
        return len([input_]) != 1 or not self.input_.isna() or self.input_.isdigit()
    ...
    def play(self):
        ...
        while True:
            ...
            user_input = Hangman.is_invalid_letter(self) # user_input 을 입력받는 메소드 적용.

            # Validate the user input
            if user_input in user_input.is_invalid_letter(self): # 플레이어가 입력한 인풋이 유효한 값인지 아닌지.
            ...
            if user_input in user_input.get_user_input():
            ...
            if user_input in self.word_to_guess:
                indexes = user_input.update_progress() # 추측한 글자의 인덱스 받기
                indexes.update  # 입력받은 글자로 현재 게임 상태 업데이트

                # If there is no letter to find in the word
                if '_' not in indexes: # 더이상 맞출 글자가 없으면
                    print('\n------Yay! You win!')
